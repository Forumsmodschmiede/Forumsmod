...
country = {
	...
	NEWSPAPER_PRINTING_FREQUENCY = 9999999, -- days frequency when the news attempt to be printed (may not print if not enough facts collected, and retry after another X days)
	NEWSPAPER_TIMEOUT_PERIOD = 9999999, -- the max period for news that may be printed. The probability is decreased with time.
...
economy = {
	...
	MAX_LOAN_CAP_FROM_BANKS = 60, -- can loan max % of country tax base from single country
...
military = {
	...
	LEADER_TRANSFER_PENALTY_ON_COUNTRY_PRESTIGE = 0.1, -- country prestige penalty on unassign leader (f.ex if leader has 100% prestige, the country loose 4% of its prestige)
...
ai =
{
	...
	ONE_SIDE_MAX_WARSCORE = 0, -- don't add too many wargoals to one side in a war
	...
	AGGRESSION_BASE = -99, -- general AI aggression
	AGGRESSION_UNCIV_BONUS = 0, -- additional AI civ aggression against uncivs
